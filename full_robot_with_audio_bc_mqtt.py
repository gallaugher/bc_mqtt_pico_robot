# MQTT Raspberry Pi Pico W Robot with BC MQTT Server & CircuitPython
# Assumes:
# - Pico's MAC address has been registered at helix.bc.edu
# - Client is running on eduroam
# - settings.toml has these values:
#  CIRCUITPY_WIFI_SSID = "BostonCollege"
#  CIRCUITPY_WIFI_PASSWORD = ""
#   BROKER="makerspaceserv.bc.edu"
#   PORT = 1884

import board, time, pwmio, busio, sdcardio, storage, os, audiobusio
import os, ssl, socketpool, wifi
import adafruit_minimqtt.adafruit_minimqtt as MQTT
from audiopwmio import PWMAudioOut as AudioOut
from audiomp3 import MP3Decoder
from adafruit_motor import servo

# define key values:
feeds = "gallaugh" # feeds entered in browser console like /gallaugh/move_feed
broker=os.getenv("BROKER")

# SPI SD_CS pin for SD card on Adalogger Cowbell
SD_CS = board.GP17

#  SPI0 setup for SD card
spi0 = busio.SPI(board.GP18, board.GP19, board.GP16)
sdcard = sdcardio.SDCard(spi0, SD_CS)
vfs = storage.VfsFat(sdcard)
try:
    storage.mount(vfs, "/sd")
    print("😎 sd card mounted")
except ValueError:
    print("❌ no SD card")

# Setup servo on GP15 left GP14 right
pwm_left = pwmio.PWMOut(board.GP13, frequency=50)
pwm_right = pwmio.PWMOut(board.GP12, frequency=50)
servo_left = servo.ContinuousServo(pwm_left)
servo_right = servo.ContinuousServo(pwm_right)

left_adjust = 0.0
right_adjust = 0.075

def move_servo(left_throttle, right_throttle):
    right_reverse = -1.0
    left_reverse = 1.0
    # servo_left.throttle = (left_throttle + left_adjust) * left_reverse
    throttle = max(min((left_throttle + left_adjust) * left_reverse, 1.0), -1.0)
    print("left_throttle:", throttle)
    servo_left.throttle = throttle

    throttle = max(min((right_throttle + right_adjust) * right_reverse, 1.0), -1.0)
    print("right_throttle:", throttle)
    servo_right.throttle = throttle

# # Setup speaker
audio = AudioOut(board.GP14) # assuming tip of Audio pin to GP16
path = "/sd/robot_sounds/" # name of folder on SD card containing audio files

# Setup MP3 decoder
filename = "0.mp3"
mp3_file = open(path + filename, "rb")
decoder = MP3Decoder(mp3_file)

# function to play an mp3 file
def play_mp3(filename):
    decoder.file = open(path + filename, "rb")
    audio.play(decoder)

# # Get adafruit io username and key from settings.toml
# aio_username = os.getenv('ADAFRUIT_AIO_USERNAME')
# aio_key = os.getenv('ADAFRUIT_AIO_KEY')
#
# # Setup a feed: This may have a different name than your Dashboard
# move_feed = aio_username + "/feeds/move_feed"
# sounds_feed = aio_username + "/feeds/sounds_feed"

# For BC MQTT Server
move_feed = f"/{feeds}/move_feed"
sounds_feed = f"/{feeds}/sounds_feed"

# Setup functions to respond to MQTT events

def connected(client, userdata, flags, rc):
    # Connected to broker at adafruit io
    print(f"Connected to {broker}! Listening for topic changes in feeds I've subscribed to on /{feeds}/")
    # Subscribe to all changes on the feed.
    client.subscribe(move_feed)
    client.subscribe(sounds_feed)

def disconnected(client, userdata, rc):
    # Disconnected from the broker at adafruit io
    print(f"Disconnected from {broker}!")

def message(client, topic, message):
    # The bulk of your code to respond to MQTT will be here, NOT in while True:
    print(f"topic: {topic}, message: {message}")
    if topic == sounds_feed: # sound button was pressed
        if message != "0": # ignore button release message "0"
            play_mp3(message)
            print("This is where I would have played sound")
    elif topic == move_feed: # robot directions
        if message == "stop":
            # stop both servos
            move_servo(0.0, 0.0)
        elif message == "forward":
            move_servo(1.0, 1.0)
        elif message == "back":
            move_servo(-1.0, -1.0)
        elif message == "left":
            move_servo(-0.5, 0.5)
        elif message == "right":
            move_servo(0.5, -0.5)

# Connect to WiFi
print(f"Connecting to WiFi")
wifi.radio.connect(os.getenv("CIRCUITPY_WIFI_SSID"), os.getenv("CIRCUITPY_WIFI_PASSWORD"))
print("Connected!")

# Create a socket pool
pool = socketpool.SocketPool(wifi.radio)

# Set up a MiniMQTT Client - this is our current program that subscribes or "listens")
mqtt_client = MQTT.MQTT(
    broker=os.getenv("BROKER"),
    port=os.getenv("PORT"),
    # username=aio_username,
    # password=aio_key,
    socket_pool=pool,
    ssl_context=ssl.create_default_context(),
)

# Setup the "callback" mqtt methods above
mqtt_client.on_connect = connected
mqtt_client.on_disconnect = disconnected
mqtt_client.on_message = message

# Connect to the MQTT broker (adafruit io for us)
print(f"Connecting to {broker}...")
mqtt_client.connect()

# play_mp3("startup.mp3")
print("We're rolling!")

while True:
    # keep checking the mqtt message queue
    try:
        # keep checking the mqtt message queue
        mqtt_client.loop()
        # If you had other non mqtt code, you could add it here.
    except MQTT.MMQTTException as e:
        print(f"MQTT Error: {e}")
        # Optional: Add a delay before reconnecting
        time.sleep(1)
        try:
            # Try to reconnect
            print("Attempting to reconnect...")
            mqtt_client.reconnect()
            print("Reconnected successfully!")
        except Exception as reconnect_error:
            print(f"Reconnection failed: {reconnect_error}")
            # Optional: Wait longer before next attempt
            time.sleep(5)
    except Exception as general_error:
        # Catch any other errors
        print(f"General error: {general_error}")
        time.sleep(1)
